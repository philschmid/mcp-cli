...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF leading-space mixed-case false as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\n False\r\n';

      const configPath = join(tempDir, 'missing_env_leading_space_mixed_case_false_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
