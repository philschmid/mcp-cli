...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF trailing-space false as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\nfalse \r\n';

      const configPath = join(tempDir, 'missing_env_trailing_space_false_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
