...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('rejects method-only server config as missing command/url shape', async () => {
      const configPath = join(tempDir, 'method_only_server.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
      ...
      await expect(loadConfig(configPath)).rejects.toThrow('Server "methodonly" missing required field');
