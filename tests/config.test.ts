...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF space-padded 00 as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\n 00 \r\n';

      const configPath = join(tempDir, 'missing_env_space_padded_double_zero_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
