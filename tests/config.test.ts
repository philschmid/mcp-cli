...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF trailing-tab uppercase false as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\nFALSE	\r\n';

      const configPath = join(tempDir, 'missing_env_trailing_tab_uppercase_false_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
