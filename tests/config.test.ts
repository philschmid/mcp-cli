...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF tab-padded true as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\n\ttrue\t\r\n';

      const configPath = join(tempDir, 'missing_env_tab_padded_true_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
