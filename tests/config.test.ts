...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-newline false as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\nfalse\n';

      const configPath = join(tempDir, 'missing_env_false_surrounded_newlines.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
