...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('rejects transport-only server config as missing command/url shape', async () => {
      const configPath = join(tempDir, 'transport_only_server.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
      ...
      await expect(loadConfig(configPath)).rejects.toThrow('Server "transportonly" missing required field');
