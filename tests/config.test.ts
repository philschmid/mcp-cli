...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF space-padded zero as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\n 0 \r\n';

      const configPath = join(tempDir, 'missing_env_space_padded_zero_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
