...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with trailing-newline mixed-case false as strict mode', async () => {
      process.env.MCP_STRICT_ENV = 'False\n';

      const configPath = join(tempDir, 'missing_env_false_trailing_newline.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
