...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF trailing-space uppercase true as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\nTRUE \r\n';

      const configPath = join(tempDir, 'missing_env_trailing_space_uppercase_true_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
