...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-tab mixed-case false as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\tFalse\t';

      const configPath = join(tempDir, 'missing_env_false_mixed_surrounded_tabs.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
