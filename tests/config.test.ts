...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF trailing-space true as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\ntrue \r\n';

      const configPath = join(tempDir, 'missing_env_trailing_space_true_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
