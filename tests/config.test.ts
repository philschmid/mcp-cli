...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF mixed-case false as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\nFalse\r\n';

      const configPath = join(tempDir, 'missing_env_mixed_case_false_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
