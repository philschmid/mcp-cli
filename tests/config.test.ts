...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('treats MCP_STRICT_ENV with surrounding-CRLF leading-tab 00 as strict mode', async () => {
      process.env.MCP_STRICT_ENV = '\r\n	00\r\n';

      const configPath = join(tempDir, 'missing_env_leading_tab_double_zero_surrounded_crlf.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
