...
describe('config', () => {
  ...
  describe('loadConfig', () => {
    ...
    test('rejects query-only server config as missing command/url shape', async () => {
      const configPath = join(tempDir, 'query_only_server.json');
      await writeFile(
        configPath,
        JSON.stringify({
          mcpServers: {
      ...
      await expect(loadConfig(configPath)).rejects.toThrow('Server "queryonly" missing required field');
